...
# 设置 object points, 形式为 (0,0,0), (1,0,0), (2,0,0) ....,(8,5,0)
...
for i in range(0, 30):  
    t = str(i)
    ChessImaR = cv2.imread('right_' + t + '.jpg', 0)  # 右视图
    ChessImaL = cv2.imread('left_' + t + '.jpg', 0)  # 左视图
    retR, cornersR = cv2.findChessboardCorners(ChessImaR, (9, 6), None)  # 提取右图每一张图片的角点
    retL, cornersL = cv2.findChessboardCorners(ChessImaL, (9, 6), None)  # 提取左图每一张图片的角点
    if retR and retL:
...
# 显示标定结果,用图像展示
# 右侧相机单独标定结果
for i in range(0, 1):  # 以第一张图片为例
    t = str(i)
    ChessImaR = cv2.imread('right_' + t + '.jpg', 0)
    hR, wR = ChessImaR.shape[:2]
    newcameramtxR, roiR = cv2.getOptimalNewCameraMatrix(mtxR, distR, (wR, hR), 1, (wR, hR))
    dstR = cv2.undistort(ChessImaR, mtxR, distR, None, newcameramtxR)
    x, y, w, h = roiR
    dstR = dstR[y:y + h, x:x + w]
    cv2.imwrite('calibresultR' + t + '.png', dstR)

# 左侧相机单独标定结果
for i in range(0, 1):  # 以第一张图片为例
    t = str(i)
    ChessImaL = cv2.imread('left_' + t + '.jpg', 0)
    hL, wL = ChessImaL.shape[:2]
    newcameramtxL, roiL = cv2.getOptimalNewCameraMatrix(mtxL, distL, (wL, hL), 1, (wL, hL))
    dstL = cv2.undistort(ChessImaL, mtxL, distL, None, newcameramtxL)
    x, y, w, h = roiL
    dstL = dstL[y:y + h, x:x + w]
    cv2.imwrite('calibresultL' + t + '.png', dstL)
...
retS, MLS, dLS, MRS, dRS, R, T, E, F = cv2.stereoCalibrate(
    objpoints, imgpointsL, imgpointsR, OmtxL, distL, OmtxR, distR, ChessImaR.shape[::-1], criteria_stereo, flags
)
...
for i in range(0, 1):  # 以第一对图片为例
    t = str(i)
    frameR = cv2.imread('right_' + t + '.jpg', 0)
    frameL = cv2.imread('left_' + t + '.jpg', 0)
